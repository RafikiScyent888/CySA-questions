...
const TILE_THEMES = {
  ...
  storm:     { emoji: '⛈️', label: 'Storm Cloud' },/* Shared utilities: session persistence, shuffling, objective metadata. */
...
function questionPool(objectiveCodes) {
  if (!objectiveCodes || objectiveCodes.length === 0) return CYSA_QUESTIONS;
  const set = new Set(objectiveCodes);
  return CYSA_QUESTIONS.filter(q => set.has(q.objective));
}
...
function objectiveBreakdown(session) {
  const stats = {};
  session.questions.forEach((q, i) => {
    if (!stats[q.objective]) {
      stats[q.objective] = { name: q.objectiveName, correct: 0, total: 0 };
    }
    stats[q.objective].total++;
    const ansIdx = session.userAnswers[i];
    if (ansIdx !== null && ansIdx !== undefined && q.options[ansIdx] && q.options[ansIdx].correct) {
      stats[q.objective].correct++;
    }
  });
  return stats;
}
